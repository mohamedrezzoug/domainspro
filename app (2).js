...
async function callClaude(prompt, { webSearch = false } = {}) {
  ...
  const body = {
    ...
    messages: [{ role: 'user', content: 'تعليمة صارمة: إذا لم تكن متأكدًا بنسبة عالية من معلومة محددة (رقم سعر، حالة تسجيل، اسم شركة بعينه، تاريخ)، اكتب "غير مؤكد" أو "لم يُعثر على معلومة موثوقة" صراحة بدل اختلاق رقم أو اسم يبدو منطقيًا. الدقة أهم من اكتمال الجواب.\n\n' + prompt }]
  };
...
function verifyLinksHTML(links) {
  return `<div class="disclaimer" style="border-style:solid; border-color:rgba(111,227,196,.4);">
    <strong style="color:var(--teal)">🔗 تحقق سريع بنفسك (بضغطة وحدة):</strong><br>
    ${links.map(l => `<a href="${l.url}" target="_blank" style="color:var(--teal)">${l.label}</a>`).join(' &nbsp;·&nbsp; ')}
  </div>`;
}
...
async function runLegal() {
  ...
  const prompt = `اسم النطاق "${state.domain}" مسجّل ومملوك بالفعل من طرف الشخص اللي كيسولك (مؤكد عبر RDAP رسمي). لا تعتبر "${state.domain}" نفسه طرفًا ثالثًا أو استخدامًا مقلقًا — ركز فقط على أطراف ثالثة أخرى بامتدادات مختلفة تمامًا (تجنب الخلط بين ${state.domain} وأي امتداد مشابه كـ .com.au أو .net في نتائج البحث).
حلل المخاطر القانونية لاسم النطاق "${state.base}" من ناحية العلامات التجارية. استخدم البحث على الويب للتحقق من: وجود علامة تجارية مطابقة، وجود علامات مشابهة قد تسبب نزاع، سوابق UDRP معروفة لأسماء مشابهة، وشركات مشهورة تستخدم هذا الاسم فعليًا (بامتدادات أو سياقات أخرى غير ${state.domain}).
...
legal_risk_score رقم من 0 إلى 100 حيث 0 = خطر منعدم و100 = خطر قانوني شديد.`;
  ...
  document.getElementById('body-legal').innerHTML = `
    ...
    ${verifyLinksHTML([
      { label: 'بحث USPTO رسمي', url: 'https://tmsearch.uspto.gov/search/search-information' },
      { label: 'WIPO Global Brand DB', url: 'https://branddb.wipo.int/en/quicksearch' },
      { label: 'بحث Google عن العلامة', url: 'https://www.google.com/search?q=%22' + encodeURIComponent(state.base) + '%22+trademark' }
    ])}
    <div class="disclaimer">هذا تحليل استرشادي بالذكاء الاصطناعي وبحث ويب عام، وليس بحث علامات تجارية رسمي. للقرارات المهمة راجع الروابط أعلاه أو محامي ملكية فكرية.</div>`;
}
...
async function runCompetition() {
  ...
  try {
    const prompt = `اسم النطاق "${state.domain}" مسجّل ومملوك بالفعل من طرف الشخص اللي كيسولك (تأكد هذا عبر RDAP رسمي، بلا شك). لا تحلل ملكية "${state.domain}" نفسه ولا تعتبره "منافس" أو "استخدام فعلي مقلق" — هو الأصل قيد التقييم، ماشي طرف ثالث.
باستخدام البحث على الويب، تحقق فقط من: هل اسم "${state.base}" (بمعزل عن الامتداد ${state.domain}) محجوز أو مستخدم فعليًا من طرف أطراف ثالثة أخرى في: منصات التواصل الاجتماعي الرئيسية (X, Instagram, TikTok)، ومواقع فعلية تعمل بامتدادات مختلفة تمامًا عن ${state.domain} (مثلاً .com.au أو .net أو .org إلخ — انتبه جيدًا لا تخلط بين الامتداد الأصلي ${state.domain} وأي امتداد آخر مشابه في نتائج البحث، هذا خطأ شائع يجب تجنبه).
أجب حصريًا بصيغة JSON:
{"social_media_usage":"...","active_competing_sites":"...","competition_score":0,"summary":"ملخص جملتين"}
competition_score من 0 إلى 100 حيث 100 = لا توجد منافسة من أطراف ثالثة (فرصة نادرة) و0 = مستخدم بكثافة من طرف أطراف أخرى. خذ بعين الاعتبار فقط السوشيال ميديا والمواقع المنافسة من أطراف ثالثة (نتائج App Store الحقيقية معروضة بشكل منفصل).`;
    const text = await callClaude(prompt, { webSearch: true });
  ...
  document.getElementById('body-competition').innerHTML = `
    ...
    ${verifyLinksHTML([
      { label: 'بحث Google عن الاسم', url: 'https://www.google.com/search?q=%22' + encodeURIComponent(state.base) + '%22' },
      { label: 'واش كاين حساب X', url: 'https://x.com/' + encodeURIComponent(state.base) },
      { label: 'واش كاين حساب Instagram', url: 'https://www.instagram.com/' + encodeURIComponent(state.base) + '/' }
    ])}`;
}
...
الاحتمالات نسب مئوية من 0 إلى 100. forecast_score من 0 إلى 100.`;
  ...
  document.getElementById('body-forecast').innerHTML = `
    ...
    <div class="summary-box">${j.summary}</div>
    ${verifyLinksHTML([
      { label: 'واش الدومين معروض للبيع (GoDaddy)', url: 'https://www.godaddy.com/domainsearch/find?domain=' + encodeURIComponent(state.domain) },
      { label: 'مبيعات دومينات مشابهة (NameBio)', url: 'https://namebio.com/search?keyword=' + encodeURIComponent(state.base) },
      { label: 'من هو المالك الحالي (WHOIS)', url: 'https://who.is/whois/' + encodeURIComponent(state.domain) }
    ])}
    <div class="disclaimer">أرقام الأسعار والاحتمالات هنا تقديرات AI عامة — قبل أي قرار بيع أو شراء تأكد من السعر الحقيقي عبر الروابط أعلاه.</div>`;
}
