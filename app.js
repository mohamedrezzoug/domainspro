...
async function callClaude(prompt, { webSearch = false } = {}) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) throw new Error('مفتاح API خاطئ أو منتهي — تحقق منه فـ "⚙️ مفتاح API"');
    if (res.status === 402) throw new Error('المشكل فالفوترة — زيد رصيد فـ console.anthropic.com');
